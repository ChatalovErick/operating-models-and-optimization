...
lh_pars["l50"] <- stock$l50
lh_pars["a50"] <- stock$a50
lh_pars["a"] <- stock$a
lh_pars["b"] <- stock$b
...
ipma_catch_age <- array(NA,dim = c(25,15,500),dimnames = list(1:25,1:15,1:500))
...
for (i in 1:500){
  for (y in 1:15) {
    for (a in 1:25){
      ...
      if (a < 25) {
        ipma_catch_age[a,y,i] <-sum(data_ipma[between(data_ipma$Ltclass,c(length_at_age[a,1,1,1,1,i]),c(length_at_age[a+1,1,1,1,1,i])),y+1])
        
      } else {
        ipma_catch_age[a,y,i] <- sum(data_ipma[between(data_ipma$Ltclass,c(length_at_age[a,1,1,1,1,i]),Inf),y+1])
      }
...
###########################################################################
# aggregate data 
matrix <- matrix(0,nrow=params$linf-nrow(data_ipma),ncol=ncol(data_ipma))
colnames(matrix) <- names(data_ipma)
data_ipma <- rbind(data_ipma,matrix)

data_ipma <- apply(data_ipma,2,function(x){round(x,1)})
...
aggregate_data <- function(data,bin_size,linf){
  ...
  for (len in 1:(length(interval_val)-1)){
    mid_lengths[len] <- (interval_val[len]+interval_val[len])/2
    C[len,] <- colSums(data[c(interval_val[len]:(interval_val[len+1])),])
  }
...
# data_ipma_bin_size_4 <- data_ipma

###########################################################################
###########################################################################
# parameters for the catch curve from the distribution of the FLCore 

data <- list()
### age and length at 50% selectivity ###
t50S <- c()
t95S <- c()
l50S <- c()
l95S <- c()
for(i in 1:500){
  Linf <- c(params["linf"])[i]
  K <- c(params["k"])[i]
  t0 <- c(params["t0"])[i]
  M <- stock$M
  Lmat <- stock$l50
  tmax <- stock$a50
  sample.no <- 1:nrow(data_ipma_bin_size_4)
  midLengths <- as.numeric(rownames(data_ipma_bin_size_4))
  catch <- data_ipma_bin_size_4
  
  values <- list("Linf"=Linf,"K"=K,"t0"=t0,"M"=M,"Lmat"=Lmat,"tmax"=tmax,
                 "sample.no"=sample.no,"midLengths"=midLengths,"catch"=catch)
  
  res_cc <- catchCurve(values,catch_columns = c(1:15), auto=TRUE, calc_ogive = TRUE)
  wqs <- (res_cc$L75 - res_cc$L50)*2
  
  values$L50 <- res_cc$L50
  values$t50 <- res_cc$t50
  data[[i]] <- values
  t50S <- c(t50S,res_cc$t50)
  t95S <- c(t95S,res_cc$t95)
  l50S <- c(l50S,res_cc$L50)
  l95S <- c(l95S,res_cc$L95)
}
...
lh_pars[c("sel1")] <- array(t50S)
lh_pars[c("sel2")] <- rep(1,500) 
lh_pars[c("sel3")] <- rep(5000,500)
