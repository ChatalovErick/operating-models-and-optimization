...
scenario <- "OM 3"
fhist <- "one-way"
calculate_sel_params = TRUE
#sel_params <- c(4.9,1,9) # parameters used
...
library(mse)
library(foreach)
source("operating model 3/om_selectivity_estimation.R")
source("funs.R")
...
################################################################
################################################################
######### catch at age from ipma ###########################
ipma_data <- read.csv("input/RJC_numbers_IC_all.csv", header = TRUE, sep = ";")
ipma_data1 <- ipma_data[,2:17]

ipma_catch_age <- matrix(NaN,nrow=length(1:25),ncol=15)
colnames(ipma_catch_age) <- c(colnames(ipma_data1[,c(2:16)]))
length_at_age <- 128*(1-exp(-(lh_pars["k"]*(1:25-lh_pars["t0"]))))

for (y in 1:15){
  for (a in 1:length(length_at_age)){
    
    if (a < 25) {
      ipma_catch_age[a,y] <-sum(ipma_data1[between(ipma_data1$Ltclass,length_at_age[a],length_at_age[a+1]),y+1])
      
    } else {
      ipma_catch_age[a,y] <- sum(ipma_data1[between(ipma_data1$Ltclass,length_at_age[a],Inf),y+1])
    }
    
  }
}

ipma_catch_age_comulative<-apply(apply(ipma_catch_age, 2, function(x){
  res <- x/sum(x)
  return(res)}),2,function(x){
    res <- cumsum(x)
    return(res)
  })

ipma_prop_Catch <- apply(ipma_catch_age, 2, function(x){
  res <- x/sum(x)
  return(res)
})
...
if(OM_scn$selectivity == "logistic"){
  sel_def <- FLifeE::logistic_Sel
} else if(OM_scn$selectivity == "dnormal") {
...
source("operating model 3/om_selectivity_estimation.R")
set.seed(0)
if (isTRUE(calculate_sel_params)){
  path <- "operating model 3/output/dnormal_pso/"
  # dnormal #
  dnormal_params <- parallelPSO::pso(fitness_catch,number_parameters = 3,number_of_partiples = 50,max_number_iterations = 30,
                                            parameters_bounds = cbind(c(1,1,1),c(15,5000,5000)),parallel=cl1,sel_f = sel_def,cumsum=FALSE,ipma_catch_age=ipma_catch_age,fhist=fhist,path=path)
  
  lh_pars[c("sel1","sel2","sel3")] <- dnormal_params$par
} else {
  
  ### change manually 
  lh_pars[c("sel1","sel2","sel3")] <- sel_params
}
...
sel_dnormal <- c(FLife::dnormal(FLQuant(1:max_age,dimnames=list("age"=1:max_age)),lh_pars))
