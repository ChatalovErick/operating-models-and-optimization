...
mp_fitness <- function(par, inp_file, path, check_file = TRUE,scenario,
  ...
  if (identical(stat_yrs, "all") | identical(stat_yrs, "more")) {
    ...
    SSB_rel <- stats[[8]] #stats["SSB_rel", ]
    Catch_rel <- stats[[10]] #stats["Catch_rel", ]
    Fbar_rel <- stats[[9]] #stats["Fbar_rel", ]
    risk_Blim <- stats[[1]] #stats["risk_Blim", ]
    ICV <- stats[[11]] #stats["ICV", ]
  ...
  # used before sum(abs(unlist(Catch_rel) - 1))
