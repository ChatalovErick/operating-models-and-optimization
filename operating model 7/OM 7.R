...
library(foreach)
library(FLa4a)
source("operating model 7/om_selectivity_estimation bayesian.R")
source("funs.R")
...
# initialization for the parellel computing #
n_workers <- 2
...
if (isTRUE(calculate_sel_params)){
  ...
  ipma_catch_age <- array(NA,dim = c(25,15),dimnames = list(1:25,1:15))
  ...
  # Get the lengths through different times of the year for the 500 iter
  length_at_age   <- growth(age+m.spwn,params) # slen is length at spawning time
  length_at_age <- apply(length_at_age,c(1),median)
  ...
  ipma_prop_Catch <- apply(ipma_catch_age, c(2), function(x){
    res <- x/sum(x)
    return(res)
  })
  
  # mean catch from ipma data #
  ipma_prop_Catch <- FLQuant(ipma_prop_Catch,dimnames = list(quant=1:25,year=1:15))

  ##############################################################################
  ##############################################################################
  
  inp_file <- tempfile()
  saveRDS(object = lh_pars1, file = inp_file, compress = FALSE)
  gc(reset = TRUE)
  
  path <- "operating model 7/output/dnormal_pso/"
  # dnormal #
  set.seed(0)
  dnormal_params <- parallelPSO::pso(fitness_catch,number_parameters = 3,number_of_partiples = 50,max_number_iterations = 30,
                                     parameters_bounds = cbind(c(1,1,1),c(15,5000,5000)),parallel=cl1,sel_f = sel_def,catch_ipma_prop=ipma_prop_Catch,fhist=fhist,path=path,inp_file=inp_file)
